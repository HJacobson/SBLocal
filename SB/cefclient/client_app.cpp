...
extern std::string StartupURL;
extern bool CacheBrowserToMemory;
...
void ClientApp::TestCookieFunctionality(int WhatToDo)
{
	if(WhatToDo == 0)
	{
		ClientApp::CreateTestCookie("MyTestCookieName", "MyTestCookieValue");
	}
	else if (WhatToDo == 1)
	{
		ClientApp::DeleteTestCookie();
	}
}

// cookies and cache are stored in the same place.
// SetStoragePath("") uses memory for cookies and cache.

void ClientApp::SetStoragePath(void)
{
	CefRefPtr<CefCookieManager> manager = CefCookieManager::GetGlobalManager();
	ASSERT(manager.get());

	if(CacheBrowserToMemory)
	{
		manager->SetStoragePath("");
	}
	else
	{
		DeleteCookiesDirectory();

		std::string CookieDirPath = "";

		if(CreateCookiesDirectory(CookieDirPath))
		{
			if(CookieDirPath != "")
			{
				manager->SetStoragePath(CookieDirPath);
			}
			else
			{
				manager->SetStoragePath("");
...
void ClientApp::CreateTestCookie(std::string CookieName, std::string CookieValue)
{
	CefString(&MyTestCookie.name).FromString(CookieName);
	CefString(&MyTestCookie.value).FromString(CookieValue);

	CefRefPtr<CefCookieManager> manager = CefCookieManager::GetGlobalManager();
	ASSERT(manager.get());
	CefString url = StartupURL;

	try
	{
		CefPath =
			"C:\\Documents and Settings\\uggy\\My Documents\\FLEXION - Software\\CEF3\\DRCBrowser\\Cookies";
		
		manager->SetStoragePath(CefPath);

		manager->SetCookie(url, MyTestCookie);
	}
...
// Henry
void ClientApp::DeleteCookies(void)
{
}
