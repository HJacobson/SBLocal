...
void SetDRCConfiguration(HWND hWnd);

// The global ClientHandler reference.
extern CefRefPtr<ClientHandler> g_handler;

extern bool AllowAltTab;
extern bool AllowMouseRightClick;
extern bool ClearClipboardOnBegin;
extern bool ClearClipboardOnExit;
extern bool HideTaskBar;
extern bool ShowWinMaximized;
...
int APIENTRY wWinMain(HINSTANCE hInstance,
  ...
  HHOOK hhkLowLevelKybd  = SetWindowsHookExA(WH_KEYBOARD_LL, LowLevelKeyboardProc, hInstance, 0);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  if(ShowWinMaximized)
  {
	hWnd = CreateWindow(szWindowClass, szTitle,
              WS_OVERLAPPED | WS_CLIPCHILDREN | WS_MAXIMIZE | WS_THICKFRAME | WS_CAPTION,
			  CW_USEDEFAULT, 0,
...
void SetDRCConfiguration(HWND hWnd)
{
  ...
  if(!AllowAltTab)
  {
	// Note: 199 is a unique number
	BOOL IsAltTabTrapped = RegisterHotKey(hWnd, 199, MOD_ALT, VK_TAB);
	// To unlock: UnRegisterHotKeyhWnd, 100);
  }

  if(ClearClipboardOnBegin)
  {
	  ClearClipboard();
  }
}
...
else if (message == WM_SYSKEYUP)
{
  ...
  // Henry: This works for right mouse clicks, except for the web page
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	OutputDebugString(L"LowLevelKeyboardProc\n");
